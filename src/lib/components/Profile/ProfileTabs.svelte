<script>
  ...
  import { authStore } from "$lib/stores/authStore";
    import MyListings from "./MyItems/MyListings.svelte";
  export let user;
  console.log(user);
  ...
  onMount(() => {
    ...
    if (totalCarbonFootprint >= carbonThresholds.Expert) {
      ...
      level = "Advanced";
      carbonSavings =
        ((totalCarbonFootprint - carbonThresholds.Advanced) /
          (carbonThresholds.Expert - carbonThresholds.Advanced)) *
        100;
    } else if (totalCarbonFootprint >= carbonThresholds.Intermediate) {
      level = "Intermediate";
      carbonSavings =
        ((totalCarbonFootprint - carbonThresholds.Intermediate) /
          (carbonThresholds.Advanced - carbonThresholds.Intermediate)) *
        100;
    } else if (totalCarbonFootprint >= carbonThresholds.Beginner) {
      level = "Beginner";
      carbonSavings =
        ((totalCarbonFootprint - carbonThresholds.Beginner) /
          (carbonThresholds.Intermediate - carbonThresholds.Beginner)) *
        100;
    } else {
      level = "Beginner";
  ...
  async function fetchMyItems(userId) {
    try {
      const response = await fetch(
        `http://localhost:3017/items/user/${userId}`,
      );
      if (!response.ok) throw new Error("Failed to fetch user's items.");
      const jsonResponse = await response.json();
      myItems = jsonResponse.data; // Extract only the 'data' array
      console.log("User's items:", myItems); // Verify the extracted data
    } catch (error) {
...
<div class="profile-container">
  <div class="tabs">
    <button
      class:active={activeTab === "my-items"}
    ...
    <button
      class:active={activeTab === "dealed-items"}
      on:click={() => (activeTab = "dealed-items")}
    >
      Dealed Items
    </button>
    <button
      class:active={activeTab === "carbon-footprint"}
      on:click={() => (activeTab = "carbon-footprint")}
    >
      Carbon Footprint
    </button>
  </div>
  ...
  {:else if activeTab === "carbon-footprint"}
    <div class="carbon-footprint-section">
      ...
      <div class="footprint-stats">
        <p>
          You have reduced <strong>{totalCarbonFootprint} kg CO2</strong> emission
        </p>
        <p>To Next Level: <strong>{Math.floor(carbonSavings) || 0}%</strong></p>
        <div class="progress-bar">
          <div
            class="progress"
            style="width: {Math.floor(Math.min(carbonSavings, 100))}%"
          ></div>
        </div>
      </div>
    </div>
